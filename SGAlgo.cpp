...
bool fitGaussian(int x, int y, Vec3b pix) {
	...
	float blue = pix.val[0];
	float green = pix.val[1];
	float red = pix.val[2];

	//printf("blue la %f \n green la %f \n red la %f", blue, green, red);
	if (x == 100 && y == 100) {
	...
	for(int i = 0 ; i < k ; ++i) {
		if(match(blue,muBlue[x][y][i],sigma[x][y][i]) && match(green,muGreen[x][y][i],sigma[x][y][i]) && match(red,muRed[x][y][i],sigma[x][y][i])) {
			...
			float rho=alpha*(1.0/(pow (2.0*3.14*sigma[x][y][i]*sigma[x][y][i], 1.5)))*exp(-0.5*(pow(((float)blue - muBlue[x][y][i]), 2.0) + pow(((float)green - muGreen[x][y][i]), 2.0) + pow(((float)red - muRed[x][y][i]), 2.0))/pow(sigma[x][y][i], 2.0));
			//printf("r_ho la %f ", rho);
